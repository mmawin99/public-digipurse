...
const WalletWrapper = ({
  ...
  return (
    ...
    flex flex-col justify-center bg-black/50 z-100
    `}>
      ...
      <div className="absolute w-full h-full rounded-xl -z-10 sm:translate-y-5 afterMS:translate-y-7" style={{
        backgroundPosition: "center",
        // backgroundColor: "#ffffff",
        backgroundSize: "contain",
        backgroundImage: theme == "dark" ? CloudPattern("afafaf") : CloudPattern("303030")
      }} />
      <div className='z-10 w-full h-full flex flex-col justify-between p-6 afterMS:p-4 rounded-b-2xl'>
        <div className="flex flex-row gap-2 items-center">
          <div className='text-black bg-white shadow-sm shadow-black w-max p-2 rounded-xl aspect-square text-3xl md:text-2xl sm:text-xl flex
          items-center justify-center'>
            <i className={`${WalletIcon}`} />
          </div>

            <div className="font-bold text-xl text-black bg-white shadow-white shadow-sm px-4 rounded-full py-2 md:text-lg sm:text-base ms:text-sm xs:text-xs">{WalletName}</div>
        </div>
