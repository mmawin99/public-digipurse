...
const TransactionCategoryEdit = ({lang, user, backFunction, cacheData, editID}) => {
    console.log(cacheData[0].group.data[editID], editID);
    const [isIconSelectorGroupTxnShow,setIsIconSelectorGroupTxnShow] = useState(false);
    const [GroupTxnAddIcon,setGroupTxnAddIcon] = useState(cacheData[0].group.data[editID].icon);
    const [GroupTxnAddName,setGroupTxnAddName] = useState(cacheData[0].group.data[editID].name);
    const [GroupTxnAddColor,setGroupTxnAddColor] = useState(cacheData[0].group.data[editID].bgColor);
    const [GroupTxnAddColorIcon,setGroupTxnAddColorIcon] = useState(cacheData[0].group.data[editID].textColor);
    const handleAddGroup = ()=>{
        if(GroupTxnAddIcon == "" || GroupTxnAddName == ""){
            ...
            MySwal.fire({
                title: (lang == "th_th" ? "กำลังแก้ไขหมวดหมู่..." : "Editing category in process..."),
                text:(lang == "th_th" ? "รอสักครู่ ขั้นตอนนี้ใช้เวลาไม่นาน" : "Wait a few seconds, This will not take long."),
                didOpen: () => {
                    MySwal.showLoading()
                    setTimeout(()=>{
                        MySwal.fire({
                            title:(lang == "th_th" ? "คุณต้องการแก้ไขหมวดหมู่นี้หรือไม่?" : "Do you want to edit this category?"),
        ...
        <TXNPrint
                        ...
                        cacheData={cacheData} cardData={
                            {"id":"UeljmIa3DcxDtPEH5wcX",
                             ...
                             confirmButtonText: (lang == "th_th" ? "แก้ไข" : "Edit"),
                             denyButtonText: (lang == "th_th" ? "ไม่แก้ไข" : "Don't edit"),
                        ...
                        if (result.isConfirmed) {
                            ...
                            MySwal.fire({
                                ...
                                didOpen: () => {
                                    MySwal.showLoading()
                                    updateData(DBName,cacheData[0].group.data[editID].id,{
                                        icon:GroupTxnAddIcon,
                                        ...
                                        writeAnUpdate(user.uid,cacheData,()=>{
                                            cacheData[2]().then(respCache=>{
                                                if(respCache.success == true){
                                                    ...
                                                    backFunction();
                                                    MySwal.fire((lang == "th_th" ? "แก้ไขหมวดหมู่สำเร็จ!" :"Editing category Successfully!"), '', 'success')
                                                }
                                            }).catch((error)=>{
                                                // console.error(error);
  ...
  return (
    <div className={`w-full pl-4 md:pl-0`}>
        ...
        <span className="text-blue-500 hover:text-blue-600 cursor-pointer" onClick={()=>{
            ...
            {
                lang == "th_th" ? "แก้ไขหมวดหมู่" : "Edit Category"
            }
        </h1>
        {isIconSelectorGroupTxnShow ? 
        <IconFinder current={GroupTxnAddColorIcon}
        heightDisabled={true} 
