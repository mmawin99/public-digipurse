...
const TransactionEdit = ({lang, user, backFunction, cacheData, editID}) => {
    const [isIconSelectorGroupTxnShow,setIsIconSelectorGroupTxnShow] = useState(false);
    const [transactionName,setTransactionName] = useState(cacheData[0].txn.data[editID].txn_title);
    const [transactionmethod,setTransactionMethod] = useState(cacheData[0].txn.data[editID].txn_method_type);
    const [txnDate, setTxnDate] = useState(new Date(cacheData[0].txn.data[editID].txn_date));
    const [customDate, setCustomDate] = useState(true);
    const [txnFireBaseID,setTxnFireBaseID] = useState(cacheData[0].txn.data[editID].id);
    // payout_recieve
    const [txnMethodCHK,setTxnMethodCHK] = useState(false);
    const [txnMethod,setTxnMethod] = useState(cacheData[0].txn.data[editID].txn_method_type == "payout_recieve" ? cacheData[0].txn.data[editID].txn_type : "i");
    const [txnAmount,setTxnAmount] = useState(""+cacheData[0].txn.data[editID].txn_amount);
    const [txnGroup,setTxnGroup] = useState(cacheData[0].txn.data[editID].txn_method_type == "payout_recieve" ? cacheData[0].txn.data[editID].txn_tag : null);
    const [txnWallet,setTxnWallet] = useState(cacheData[0].txn.data[editID].txn_method_type == "payout_recieve" ? cacheData[0].txn.data[editID].txn_wallet : null);
    
    const [txnTFAmount,setTxnTFAmount] = useState(""+cacheData[0].txn.data[editID].txn_amount);
    const [TFRTxnWallet,setTFRTxnWallet] = useState(cacheData[0].txn.data[editID].txn_method_type == "transfer" ? cacheData[0].txn.data[editID].txn_reciever_wallet : null);
    const [TFTTxnWallet,setTFTTxnWallet] = useState(cacheData[0].txn.data[editID].txn_method_type == "transfer" ? cacheData[0].txn.data[editID].txn_transfer_wallet : null);
    ...
    const handleAddTransactionPayoutRecieve = ()=>{
      ...
      txnAmount == null || txnGroup == null || txnWallet == null){
        MySwal.fire((lang == "th_th" ? "ตรวจพบข้อมูลพลาด" : "Miss information detected"),(lang == "th_th" ? "คุณไม่ได้กรอกข้อมูลทั้งหมด" : "You didn't fill all information"),'warning')
        return;
      }
      MySwal.fire({
        title: (lang == "th_th" ? "การแก้ไขธุรกรรม" : "Editing Transaction."),
        text:(lang == "th_th" ? "รอสักครู่ ขั้นตอนนี้ใช้เวลาไม่นาน" : "Wait a few seconds, This will not take long."),
        didOpen: () => {
            MySwal.showLoading()
            setTimeout(()=>{
                MySwal.fire({
                    title:(lang == "th_th" ? "ต้องการแก้ไขธุรกรรมนี้หรือไม่?" : "Want to edit this transaction?"),
                  ...
                  <TXNPrint
                    ...
                    cacheData={cacheData} cardData={
                     ...
                     confirmButtonText: (lang == "th_th" ? "ใช่ แก้ไขสิ่งนี้!" : "Yes, Editing this!"),
                ...
                if (result.isConfirmed) {
                      ...
                      MySwal.fire({
                        title: (lang == "th_th" ? "กำลังแก้ไขธุรกรรมอยู่ระหว่างดำเนินการ..." : "Editing Transaction in process..."),
                        text:(lang == "th_th" ? "รอสักครู่ การดำเนินการนี้ใช้เวลาไม่นาน" : "Wait a few seconds, This will not take long."),
                        didOpen: () => {
                            MySwal.showLoading()
                            updateData(DBName,txnFireBaseID,{
                              txn_date:txnDate.toISOString(),
                              txn_title:transactionName,
                              txn_type:txnMethod,
                              txn_method_type:"payout_recieve",
                                ...
                                writeAnUpdate(user.uid,cacheData,()=>{
                                  cacheData[2]().then(respCache=>{
                                    if(respCache.success == true){
                                      MySwal.fire((lang == "th_th" ? "แก้ไขธุรกรรมสำเร็จ!" : "Edit transaction successfully!"),'','success');
                                      backFunction();
                                    }
                                  }).catch((error)=>{
                ...
                MySwal.fire({
                  ...
                  <TXNPrint
                    ...
                    cacheData={cacheData} cardData={
                     ...
                     denyButtonText: (lang == "th_th" ? "ไม่ ไม่แก้ไข" : "No, not edit."),
                     cancelButtonText: (lang == "th_th" ? "ยกลิก" : "Cancel"), 
                     allowOutsideClick:false,
                     allowEscapeKey:false,
                     allowEnterKey:false
                }).then((result) => {
    ...
    //                       MySwal.fire((lang == "th_th" ? "ไม่สามารถรับด้วยกระเป๋าเงินเดียวกันได้" : "Cannot Receive with same wallet."),"warning");
    //                     }else{
    ...
    <div className={`w-full pl-4 md:pl-0`}>
        <h1 className='text-[2em] ms:text-[1.75em] sxs:text-[1.55em] 2xs:text-[1.25em] px-4 font-bold w-full md:text-center mb-10 md:mb-6'>
          <span className="text-blue-500 hover:text-blue-600 cursor-pointer" onClick={()=>{
            ...
            {
                lang == "th_th" ? "แก้ไขรายการธุรกรรม" : "Edit Transaction"
            }
        </h1>
        {isShowSelectGroup ? 
        <div className={`w-full pl-4 md:pl-0`}>
            ...
            <div className={``}>
              ...
              <div className={`flex flex-col w-full`}>
                  ...
                  <form className='flex flex-col gap-8'>   
                      ...
                      <label className="daisyform-control w-full">
                        <div className="daisylabel">
                          <span className="daisylabel-text">{lang == "th_th" ? "ประเภทธุรกรรม" : "Transaction type"}</span>
                        </div>
                        <div className="flex flex-row gap-2 items-center">
                        <div className="daisyjoin daisyjoin-horizontal lg:daisyjoin-horizontal rounded-full">
                          <div onClick={()=>{
                      ...
                      <div className='grid grid-cols-1 gap-2'>
                          <div onClick={handleAddTransactionPayoutRecieve} className='daisybtn daisybtn-success'> 
                              {lang == "th_th" ? "แก้ไขธุรกรรม" : "Edit Transaction"}
                          </div>
                      </div>
                  </form>
                  : ""}
                  {/* transfer */}
                  {
                    ...
                    <label className={`daisyform-control w-full ${customDate ? "" : "hidden"} `}>
                      <div className="daisylabel">
                        <span className="daisylabel-text">T{lang == "th_th" ? "วันที่ทำธุรกรรม" : "Transaction date"}</span>
                      </div>
                      <ReactDatePicker
                        className={"daisyinput daisyinput-bordered w-full"}
                        selected={txnDate}
                      ...
                      <div className={`daisyalert ${TFTTxnWallet == null ? "daisyalert-warning afterMS:flex-col" : "daisyalert-info afterMS:flex-col"} justify-between flex flex-row`}>
                        <div className="flex flex-row gap-2 items-center">
                          <i className='far fa-blanket' />
                          <span className="font-semibold tracking-tighter sxs:text-sm">{ TFTTxnWallet == null ? 
